...
${state.first ? '<hr>' : ''}
<div data-comment='${vo.id}'>${vo.name} [ ${vo.writedate}]
...
$('.btn-delete-cancel').on('click', function(){
	...
	if( $(this).text()=='취소' ){		
		...
		if( confirm('댓글 삭제') ){			
			$.ajax({
				url: 'board/comment/delete/' + div.data('comment'),
				success: function(){
					comment_list();	
				}
...
$('.btn-modify-save').on('click', function(){
	...
	if( $(this).text()=='수정' ){		
		...
		var comment = 
			JSON.stringify( {id: div.data('comment'), content: div.children('.modify').val()} );
		$.ajax({
			url: 'board/comment/update',
// 			data: { id: div.data('comment'), comment: div.children('.modify').val() },
			data: comment,
			type: 'post', //JSON타입 보내려면 post
...
function modifyStatus(div, is){
	//수정상태 : 저장/취소 버튼 보이게, 원글 안 보이게, 수정글 보이게
	//보기상태 : 수정/삭제 버튼 보이게, 원글 보이게, 수정글 안 보이게
